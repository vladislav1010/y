...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app y-514a
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app y-514a-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'y-514a-staging' does not match app name in config file 'y-514a'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name y-514a-db
  fly postgres attach --postgres-app y-514a-db --app y-514a

  fly postgres create --name y-514a-staging-db
  fly postgres attach --postgres-app y-514a-staging-db --app y-514a-staging
  ```
